...
export function readArticle(slug) {
	...
	const meta = {
		...
		tags: Array.isArray(data.tags) ? [...new Set(data.tags.filter((tag) => typeof tag === "string" && tag.trim()).map((tag) => tag.trim()))] : [],
...
export async function renderMarkdown(body) {
	...
	).replace(/<\/table>/g, "</table></div>").replace(/<pre>/g, '<pre tabindex="0" aria-label="Code example">');
}
